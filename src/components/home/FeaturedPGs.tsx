...
import { ArrowRight, Star, AlertCircle, Users, Briefcase, Home, Heart, Shield, Wifi } from 'lucide-react';
...
export function FeaturedPGs() {  // Keep original export name
  const [girlStays, setGirlStays] = useState<PGListing[]>([]);
  const [boyStays, setBoyStays] = useState<PGListing[]>([]);
  const [familyStays, setFamilyStays] = useState<PGListing[]>([]);
  const [coEdStays, setCoEdStays] = useState<PGListing[]>([]);
  ...
  const fetchStays = async () => {
    try {
      ...
      const girls = publishedStays.filter(stay => stay.type === 'girls');
      const boys = publishedStays.filter(stay => stay.type === 'boys');
      const family = publishedStays.filter(stay => stay.type === 'family');
      const coEd = publishedStays.filter(stay => stay.type === 'co-ed');
      ...
      setGirlStays(sortStays(girls).slice(0, staysPerType));
      setBoyStays(sortStays(boys).slice(0, staysPerType));
      setFamilyStays(sortStays(family).slice(0, staysPerType));
      setCoEdStays(sortStays(coEd).slice(0, staysPerType));
    ...
    return (
      <div className="mb-20 last:mb-0">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          ...
          <Link to={`/pg?type=${type}`} onClick={(e) => {
            e.preventDefault();
            window.location.href = `/pg?type=${type}`;
          }}>
      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-white to-orange-50/30">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <div className="h-4 w-24 bg-orange-200 rounded animate-pulse mx-auto"></div>
            <div className="h-8 w-64 bg-orange-200 rounded animate-pulse mt-2 mx-auto"></div>
          </div>
          ...
          {[1, 2, 3, 4].map((section) => (
            <div key={section} className="mb-16">
              <div className="animate-pulse mb-8">
                ...
                <div className="h-8 w-48 bg-orange-200 rounded"></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-white border border-orange-100 rounded-xl overflow-hidden animate-pulse">
  ...
  const hasAnyStays = girlStays.length > 0 || boyStays.length > 0 || familyStays.length > 0 || coEdStays.length > 0;
  ...
  return (
    <section className="py-16 md:py-24 bg-gradient-to-b from-white to-orange-50/30">
      <div className="container mx-auto px-4">
        ...
        {renderSection(
          "Premium Stays for Women",
          "Safe, secure, and comfortable living spaces with 24/7 security",
        ...
        {renderSection(
          "Executive Stays for Men",
          "Modern residences designed for young professionals and students",
        ...
        {renderSection(
          "Family Residences",
          "Spacious homes away from home for your loved ones",
        ...
        {renderSection(
          "Co-living Spaces",
          "Inclusive accommodations for everyone",
