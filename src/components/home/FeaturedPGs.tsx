...
import { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { ArrowRight, Star, AlertCircle, GitCompare } from 'lucide-react';
import { useCompare } from "@/contexts/CompareContext";
import { Button } from '@/components/ui/button';
import { PGCard } from '@/components/pg/PGCard';
      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-white to-orange-50/30">
        <div className="container mx-auto px-4">
          <div className="text-center py-12 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No PGs Available</h3>
            <p className="text-gray-600 mb-4">There are no accommodations available at the moment.</p>
            <Button onClick={fetchPGs} variant="outline">Refresh</Button>
          </div>
        </div>
      </section>
    );
            ...
            <Link to="/compare">
              <Button variant="outline" className="flex items-center gap-2">
                ...
                {compareList.length > 0 && (
                  <span className="bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full text-xs">
                    {compareList.length}
                  </span>
                )}
              </Button>
